#!/usr/bin/python3
import sys, json, pymysql, import_tweet, numpy as np, joblib
...
cursor.execute("SELECT id, text FROM tweets WHERE mnb_sentiment IS NULL LIMIT 5000")
...
for tweet in tweets:
  tweet_text = tweet['text'].encode('utf-8').decode('unicode_escape')
  ...
  cursor.execute("UPDATE tweets SET mnb_sentiment = %s, mnb_score = %s WHERE id = %s",
  (
    int(mnb_sentiment[0]),
